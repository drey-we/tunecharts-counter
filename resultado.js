...
let headerStats = { totalFormatted: "0 Minutes", label: "Time Listened" };
...
async function atualizarDadosDoPeriodo(isInitialLoad = !1) {
    ...
    elements.userScrobbles.innerHTML = '<div class="skeleton" style="width:100px;height:40px;"></div>';
...
async function buscarPerfil() {
    try {
        ...
        if (elements.storyTitle) elements.storyTitle.textContent = displayName;
        if (elements.sqUsername) elements.sqUsername.textContent = displayName;
        if (elements.userName) elements.userName.classList.remove("skeleton");
        const img = user.image.find((i) => i.size === "extralarge") || user.image.pop();
        if (img && img["#text"]) {
            if (elements.userFoto) elements.userFoto.src = img["#text"];
            if (elements.storyUserImg) {
                elements.storyUserImg.crossOrigin = "anonymous";
                elements.storyUserImg.src = img["#text"];
            }
            if (elements.sqUserImg) {
                elements.sqUserImg.crossOrigin = "anonymous";
                elements.sqUserImg.src = img["#text"];
            }
            if (elements.userFoto) elements.userFoto.classList.remove("skeleton");
        }
    } catch (error) {
...
async function calcularTempoOuvido() {
    try {
        ...
        headerStats.totalFormatted = formattedTotal;
...
function renderizarPreviewLista(elementId, dataList, type) {
    ...
    mainItems.forEach((item, i) => {
        ...
        htmlMain += `
            <div class="chart-item ${i === 0 ? "top-1" : ""}">
                <div>#${i + 1} - ${text}${subtext}</div>
                <div style="font-size:0.85em; opacity:0.8; margin-left:auto;">${timeStr}</div>
            </div>`;
    });
    const container = document.querySelector(`#${elementId} .lista-top`);
    if (container) container.innerHTML = htmlMain || "No data.";
}
...
async function gerarImagemFinal(format, accentColor, selectedCharts) {
    ...
    try {
        ...
        link.download = `TuneCharts-Counter-${username}-${currentPeriod}.png`;
...
function formatarListaHTML(items, limit, type, format) {
    ...
    itemsToShow.forEach((item, i) => {
        let text = item.name;
        const timeString = formatTimeShort(item.seconds);
        if (format === "story") {
            html += `
            <div class="story-item ${i === 0 ? "top-1" : ""}">
                ...
                <span style="margin-left:auto; font-weight:bold; font-size:0.9em; opacity:0.9">${timeString}</span>
            </div>`;
        } else {
            html += `
            <li class="${i === 0 ? "top-1" : ""}">
    ...
    return html || "No data calculated.";
}
function aplicarCoresDinamicas(card, accentColor, format) {
    if (format === "story") {
        card.querySelectorAll(".story-subtitle, .story-rank, .stat-disclaimer").forEach(
        ...
        card.querySelectorAll(".sq-v2-report-title, .sq-v2-list li.top-1 span").forEach(
            (el) => (el.style.color = accentColor)
        );
